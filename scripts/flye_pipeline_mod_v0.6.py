...
# we usually use to train our algorithms.
    medaka_cmd = ['{0}'.format(medaka_path), '-i', long_reads, '-d', contigs, '-o',
                  '{0}/medaka_results'.format(outdir), '-m', 'r941_min_high_g303', '-t', threads]
...
def get_arguments():
    ...
    help_group.add_argument('-V', '--version', action='version', version='%(prog)s version 0.5',
    ...
    flye_group.add_argument("--genome_size", required=True, help="Estimated genome size for assembly", default=None)
